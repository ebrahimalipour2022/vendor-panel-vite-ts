...
// Type Imports
import { GenerateVerticalMenu } from '@/layouts/materialize-layout/components/GenerateMenu';
...
const processMenuChildren = (
  ...
  return Children.map(children, (child) => {
    ...
    return child.props?.menuData ? (
      <GenerateVerticalMenu menuData={child.props.menuData} />
    ) : (
      mapFunction(child)
    );
  });
  ...
  return Children.map(children, (child) => {
    ...
    switch (child.type) {
      ...
      case HorizontalSubMenu:
        ...
        return (
          <VerticalSubMenu {...rest}>{mapHorizontalToVerticalMenu(childChildren)}</VerticalSubMenu>
        );
      case HorizontalMenu:
