...
// Type Imports
import type { MenuItemStyles } from '@/layouts/materialize-layout/@menu/types';
import type { VerticalNavState } from '@/layouts/materialize-layout/@menu/contexts/verticalNavContext';
import type { Settings } from '@/layouts/materialize-layout/@core/contexts/settingsContext';

// Util Imports
import { menuClasses } from '@/layouts/materialize-layout/@menu/utils/menuClasses';
...
const menuItemStyles = (
  ...
  // Vars
  const { isCollapsed, collapsedWidth, isHovered, isPopoutWhenCollapsed, transitionDuration } =
    verticalNavOptions;
  ...
  return {
    root: ({ level }) => ({
      color: 'var(--mui-palette-customColors-menuItemColor)',
    ...
    button: ({ level, active }) => ({
      ...
      ...((!isPopoutWhenCollapsed || popoutExpanded || (popoutCollapsed && level === 0)) && {
        ...
        paddingInlineStart: theme.spacing(
          collapsedNotHovered ? ((collapsedWidth as number) - 47) / 8 : 3
        ),
